...
from adacache_core import AdaCacheEvaluator
...
def write_summary_tables(all_results, output_path, run_id):
    ...
    def make_key(row):
        ...
        return (
            ...
            row.get('dataset', ''),
            row.get('subset', ''),
            row.get('global_pool_size', ''),
            row.get('entropy_threshold', ''),
            row.get('window_size', ''),
            row.get('paper_k_full', '')
        )
...
def load_model(model_name: str, model_root: str, device_kind: str, use_sharding: bool = True):
    ...
    # 检测模型类型，决定是否使用 trust_remote_code
...
def run_adacache_experiment(
    ...
    Args:
        ...
        tasks: 任务列表（逗号分隔）
    """
    # 检测设备（确保 config.device 为 None 或有效值时才使用）
    ...
    # 创建层级目录: outputs/adacache_dataset/run_id/model_name/
    run_mode = f"adacache_{dataset_id}"
    run_root = os.path.join(config.output_dir, run_mode, config.run_id, model_id)
    os.makedirs(run_root, exist_ok=True)
    
    # 创建 latest 软链接（指向 run_id 层级）
    try:
    ...
    # 文件命名: model_tau_w_timestamp
    file_prefix = f"{model_id}_tau{config.entropy_threshold}_w{config.window_size}_{timestamp}"
    output_prefix = os.path.join(run_root, file_prefix)
    
    # 运行评估
    logging.info("=" * 80)
    logging.info(f"开始 AdaCache 评估")
    ...
    logging.info(f"  信息熵阈值: {config.entropy_threshold}")
    logging.info(f"  窗口大小: {config.window_size}")
    logging.info(f"  Paper K_full: {config.paper_k_full}")
    logging.info("=" * 80)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="AdaCache 自适应示例选择评估")
    
    # 核心参数
    parser.add_argument("--global_pool_size", type=int, default=1024, help="全局示例池大小")
    parser.add_argument("--window_size", type=int, default=4, help="每轮激活的示例数量")
    parser.add_argument("--entropy_threshold", type=float, default=0.5, help="信息熵阈值")
    parser.add_argument("--max_examples", type=int, default=512, help="最大示例数量")
    parser.add_argument("--paper_k_full", type=int, default=4, help="Paper 模式固定完整示例数")
    
    # 模型与数据集
    parser.add_argument("--models", type=str, default="/home/models/models/Qwen/Qwen2.5-14B-Instruct",
                       help="模型路径（多个用逗号分隔）")
    ...
    # 运行配置
    parser.add_argument("--run_id", type=str, default="adacache_test", help="运行 ID")
    parser.add_argument("--verbose", action="store_true", help="详细日志")
    
    args = parser.parse_args()
    
    # 设置日志（先设置基本日志，避免后续日志调用失败）
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(os.path.join(log_dir, f"adacache_{args.run_id}.log"))
        ]
    )
    ...
    # 按数据集组织结果（每个数据集有自己的汇总表）
    dataset_results = {}  # {dataset_id: {"results": [], "summary_root": ""}}
    ...
    for model_name in models:
        for dataset_config in datasets:
            ...
            try:
                ...
                metrics = run_adacache_experiment(
                    config=config,
                ...
                # 将结果添加到对应的数据集
                dataset_results[dataset_id]["results"].append({
                    "model": model_name,
    ...
    for dataset_id, data in dataset_results.items():
        ...
        if not results:
            logging.warning(f"数据集 {dataset_id} 没有实验结果")
            continue
        ...
        for result in results:
            ...
            logging.info(f"    平均示例数: {result['metrics']['optimal_k_mean']:.2f}")
            logging.info(f"    示例数范围: [{result['metrics']['optimal_k_min']}, {result['metrics']['optimal_k_max']}]")
        
        # 生成汇总表（CSV 和 Markdown）- 保存在 run_id 目录下
